...
int main(void)
{
    ...
    for(i=0,j=1;i<4;i++) //feeding colourcode
    {
        if(i%4==0)
        {
            j++;
...
then decreasing the choice.*/
        ...
        {
            tokens[unique_id][1]=colourcode+1; //assigning the area to new token
            tokens[unique_id][2]=14; //assigning the location to new token
